...
function handleEditing(e: KeyboardEvent, selected: string) {
  const { selection, setSelection, setDesiredColumn } =
...
function handleNavigation(e: KeyboardEvent, selected: string) {
  const { visualSelection, setVisualSelection, setDesiredColumn } =
    useEditorStore.getState();
...
const navigationHandlers: Record<
  string,
  (...args: Parameters<typeof handleEnd>) => VisualSelection
> = {
