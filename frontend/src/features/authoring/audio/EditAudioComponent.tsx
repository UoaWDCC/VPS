    ...
    return (
        <div className="bg-base-300 mt-xs px-[1rem] py-[0.5rem] relative">
            <div className="absolute top-2 right-2" >
                <button className="btn btn-xs btn-phantom" onClick={togglePlayback}>{playing ? "Pause" : "Play"}</button>
                <button className="btn btn-xs btn-phantom" onClick={deleteAudioComponent}>Delete</button>
            </div>
            <fieldset className="fieldset w-full mt-[0.5rem]">
                ...
                <label className="label">
                    <input type="checkbox" checked={loop} onChange={e => saveLoop(e.target.checked)} className="toggle" />
                    Loop Audio
                </label>
            </fieldset>
        </div>
