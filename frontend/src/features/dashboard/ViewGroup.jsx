...
export default function ViewGroupPage() {
    ...
    const {nodes, edges, groupEdges} = useMemo(() => {
        ...
        if(Array.isArray(scenes) && scenes.length != 0)
        {
            ...
            {
                const id = sceneMap[groupPath[i]].name + "-" + sceneMap[groupPath[i+1]].name
         ...
         <ScreenContainer vertical>
            ...
            {Object.keys(groupInfo) != 0 && (
                <div className="h-full px-10 py-7 pb- overflow-y-scroll lg:flex sm:flex-row">
                    <div className="pb-5">
                        <h1 className="text-3xl font-mona font-bold my-3">Viewing Group {groupInfo.users[0].group}</h1>
                        <TestTable groupInfo={groupInfo}/>
                    </div>
                    <div className="w-full h-full">
                        <h1 className="text-3xl font-mona font-bold px-10 my-3">Path Overview</h1>
                        <div className="relative h-[95%] w-full m-auto px-10">
                            {graphLoading && (
                                <div className="absolute h-[80%] w-full top-6 left-0 px-10">
                                    <Skeleton animation="wave" width="100%" height="100%" variant="rectangular" sx={{bgcolor: "#f2f2f2"}}/>
                                </div>

                            )}
                            {Array.isArray(scenes) && scenes.length != 0 &&Array.isArray(nodes) && nodes.length > 0 && Array.isArray(edges) && edges.length > 0 && (
                                <div className={`h-[80%] ${graphLoading ? "opacity-0" : "opacity-100"}`}>
                                    <ReactFlowProvider>
                                        <ScenarioGraph inNodes={nodes} inEdges={edges} inGPathEdges={groupEdges} onLoaded={() => {setGraphLoading(false)}}/>
                                    </ReactFlowProvider>
                                </div>
