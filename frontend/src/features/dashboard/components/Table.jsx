    ...
    return(
        <div className={classes.root}>
            <Paper>
                ...
                <TableContainer>
                    <Table className={`lg:${mode == "groups" ? "min-w-[95vw]" : "min-w-[40vw]"} sm:min-w-[80vw]`}>
                        <TableHead>
                            <TableRow>
                                {mode == "groups" ? (
                                    <>
                        ...
                        <TableBody>
                            {mode == "groups" ? group.map((ginfo, index) => (
                                 <TableRow key={ginfo._id || index}>
                                    {/* {console.log(ginfo)} */}
                                    <TableCell component="th" scope="row">{ginfo.users[0].group}</TableCell>
                                    <TableCell>{ginfo.users.length}</TableCell>
                                    <TableCell>
                                        {ginfo.users.map((user) => (
                                            <div key={user.email}>{user.name} - [{user.role}]</div>
                                        ))}
                                    </TableCell>
                                    <TableCell>{ginfo.path.length != 0 ? "Started" : "Not yet started"}</TableCell>
                                    {/* <TableCell></TableCell> */}
                                    <TableCell className="flex justify-center items-center">
                                        <Visibility className="hover:cursor-pointer" onClick={() => {
                                            rowClick(ginfo._id)
                                        }}/>
                                    </TableCell>
                                </TableRow>
