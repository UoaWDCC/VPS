...
export default function ScenarioGraph({inNodes, inEdges, inGPathEdges, onLoaded}) {
     /**
     * Adapted ELkjs code 
     * Accessed: 10/09/2025
    ...
    const elkOptions = {
        ...
        "elk.layered.spacing.nodeNodeBetweenLayers": direction === "RIGHT" ? "300" : "200",
    ...
    useEffect(() => {
        if(measured || nodes.length == 0 || !nodes.every((node) => node.measured)) return;
    ...
    return (
        <div className="h-full">
            <span className="text-sm opacity-80">NB: Node positions are not saved if moved around</span>
            <ReactFlow 
                className={`border`}
                nodes={nodes}
                edges={edges}
            ...
            >
                ...
                <Panel position="top-right" className="flex flex-col gap-2">
                    <button className="bg-white text-sm border rounded-lg py-0.5 px-1 hover:cursor-pointer" onClick={() => {setDirection((prev) => prev === "DOWN" ? "RIGHT" : "DOWN")}}>Flip</button>
                    <button className="bg-white text-sm border rounded-lg py-0.5 px-1 hover:cursor-pointer" onClick={() => {RenderGraph()}}>Reset Nodes</button>
                </Panel>
                <Background/>
            </ReactFlow>
            </div>
