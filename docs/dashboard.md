...
The [tracked values](/editor/#tracked-values) under the **State Variables** section are the current values at this point in time. You can use these to check things like player score at the end of a playthorugh, but also as an easier way to track the path. For example, instead of analysing the flowchart for every group to see if they visited a certain scene, you can use a tracked value like *has_visited_scene_x*, and just look at this for every group.
